        ...
        if (tipe === 'detail') {
            ...
            if (mainData.length > 0) {
                ...
                addTotalRow(ws, endRow + 2, 7,
                    { 5: { formula: `SUM(E${startRow}:E${endRow})` }, 7: { formula: `SUM(G${startRow}:G${endRow})` } },
                    { 5: '#,##0', 7: currencyFmt }
                );
            }
            ws.getColumn(1).width = 12; ws.getColumn(2).width = 18; ws.getColumn(3).width = 20;
            ws.getColumn(4).width = 25; ws.getColumn(5).width = 8; ws.getColumn(6).width = 15; ws.getColumn(7).width = 18;

        } else if (tipe === 'pelanggan') {
            const ws = workbook.addWorksheet('Per Pelanggan');
            addTitleBlock(ws, 'LAPORAN PER PELANGGAN', 7);
            ws.addRow(['Nama Pelanggan', 'Email', 'Telepon', 'Jumlah Pesanan', 'Total Belanja', 'Total Modal', 'Laba Bersih']);
            styleHeaderRow(ws, 4, 7);

            let totalModalAll = 0;
            let totalUntungKotor = 0;

            const startRow = 5;
            mainData.forEach((row, i) => {
                const belanja = parseFloat(row.total_belanja);
                const modal = parseFloat(row.total_modal || 0);
                const laba = belanja - modal;
                
                totalModalAll += modal;
                totalUntungKotor += belanja;
                ...
                for (let c = 1; c <= 7; c++) {
                    ...
                    else if (c === 4) { cell.numFmt = '#,##0'; cell.alignment = { horizontal: 'right', vertical: 'middle' }; }
            ...
            ws.getColumn(1).width = 25; ws.getColumn(2).width = 25; ws.getColumn(3).width = 15;
            ws.getColumn(4).width = 15; ws.getColumn(5).width = 18; ws.getColumn(6).width = 18; ws.getColumn(7).width = 18;

        } else if (tipe === 'bulanan') {
            const ws = workbook.addWorksheet('Laporan Bulanan');
            addTitleBlock(ws, 'LAPORAN BULANAN', 6);
            ws.addRow(['Bulan', 'Tahun', 'Jumlah Pesanan', 'Total Pendapatan', 'Total Modal', 'Laba Bersih']);
            ...
            const startRow = 5;
            mainData.forEach((row, i) => {
                const pend = parseFloat(row.total_pendapatan);
                const modal = parseFloat(row.total_modal || 0);
                const laba = pend - modal;

                totalModalAll += modal;
                totalUntungKotor += pend;
            ...
            ws.getColumn(1).width = 15; ws.getColumn(2).width = 12; ws.getColumn(3).width = 18;
            ws.getColumn(4).width = 25; ws.getColumn(5).width = 20; ws.getColumn(6).width = 20;

        } else {
            // ringkasan (default)
            const ws = workbook.addWorksheet('Ringkasan Penjualan');
            addTitleBlock(ws, 'LAPORAN (TAHUNAN) DETAIL PENJUALAN', 7);
            ws.addRow(['Periode', 'Nama Barang', 'Jumlah Terjual', 'Harga Jual Satuan', 'Harga Beli Satuan', 'Subtotal Penjualan', 'Laba per Item']);
            styleHeaderRow(ws, 4, 7);
            ...
            mainData.forEach((row, i) => {
                ...
                const periodeLabel = `${tgl_mulai} s/d ${tgl_selesai}`;
                ...
                for (let c = 1; c <= 7; c++) {
                    ...
                    cell.font = textFont;
                    cell.border = borderStyle;
                    cell.alignment = { vertical: 'middle', horizontal: (c === 1 || c === 2) ? 'center' : 'right' };
                }
                if (typeof hargaJualLabel === 'number') ws.getCell(r, 4).numFmt = currencyFmt;
                ws.getCell(r, 5).numFmt = currencyFmt;
                ws.getCell(r, 6).numFmt = currencyFmt;
                ws.getCell(r, 7).numFmt = currencyFmt;
            });
            ...
            ws.getColumn(1).width = 25; ws.getColumn(2).width = 30; ws.getColumn(3).width = 15;
            ws.getColumn(4).width = 20; ws.getColumn(5).width = 20; ws.getColumn(6).width = 25; ws.getColumn(7).width = 20;
        }
